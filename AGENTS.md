...
- The self-paint check (`src/self-paint.js`, surfaced by `open`/`export`/`share` as `self_paint_warning`) is render-free and fails open by design: any stylesheet link, `@import`, Tailwind runtime script, `color-scheme`, or html/body/:root background signal suppresses it, because a false warning on every open is worse than a miss that the SELF_PAINT_RULE guidance and render-verify workflow still cover. Keep it a warning - never a blocked open, never auto-repair.

...
test("a disconnect during immediate feedback take requeues the batch without working presence", async () => {
  ...
  try {
    ...
    // The batch never reached the agent, so the conversation must look exactly as it did
    // before the closed poll took it - still "sent", not "delivered".
    ...
    assert.equal(feedback.delivered_prompt_ids, undefined);
    // The re-poll is a real delivery, so now the same records report "delivered".
    assert.deepEqual(
      JSON.parse(await readFile(stateFile, "utf8")).sessions[key].chat,
      beforeState.chat.map((item) => ({ ...item, state: "delivered" })),
    );
  } finally {

...
// Restored drafts pass through the same attachment sanitizer as freshly enqueued
// prompts, so a malformed ref can never reach render() and wedge the tab.
const queued = initialQueuedPrompts.map(sanitizeQueuedPrompt).filter(Boolean);
...
// dropped: the queue is serialized to the server BEFORE it renders, so one bad
// entry would throw out of render(), stay in the session draft, and throw again on
// every reload - wedging the tab permanently instead of failing once. The card's
// own flow only ever produces `{id, name}`, so a malformed entry is fabricated and
...
// the artifact's own object through would carry that junk into the sync body and

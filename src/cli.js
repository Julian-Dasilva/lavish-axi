...
export async function ensureServer({
  ...
  const forceForExisting = forceRestart && !(localBuild && serverVersion !== VERSION);
  if (existing && !shouldRestartServer(VERSION, existing, forceForExisting, serverVersion)) {
    return baseUrl;
  }
  ...
  if (existing) {
    ...
    await shutdownRequester(baseUrl, { reloadKey, reason: serverReplacementReason(VERSION, existing, forceRestart) });
  ...
  while (Date.now() < deadline) {
    ...
    if (health?.network_stale === true && health.app === "lavish-axi") {
      ...
      await shutdownRequester(baseUrl, { reloadKey, reason: "" });
      if (!(await portFreeWaiter(baseUrl, 3000))) break;
      await serverStarter(port);
    ...
    if (
      (replacementAttempted || (localBuild && serverVersion !== VERSION)) &&
      hasServerVersionMismatch(serverVersion, health)
    ) {
...
export function createServerVersionMismatchError({ port, expectedVersion, serverVersion, commandLine }) {
  ...
  return new AxiError(
    `Lavish Editor server version mismatch: expected server ${expectedVersion}, running server ${runningVersion}; command line: ${runningCommand}`,
    "SERVER_ERROR",
    [`Run \`lavish-axi server --port ${port}\` to inspect server startup`],
  );
}

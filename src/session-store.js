...
function normalizeQueuedPrompts(prompts) {
  ...
  return prompts
    ...
    .map((prompt) => {
      // `normalizePrompt` returns `{ prompt, malformed }`; a stored draft keeps only
      // the normalized prompt - malformed attachment refs are rejected at submit time.
      const { prompt: normalized } = normalizePrompt(prompt);

...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  After an answer tied to `data-lavish-question` is accepted, Lavish collapses that question to an accessible answered summary with a native **Reopen** button; the answered state is kept per session and survives reloads.
  ...
  Sent feedback shows **sent** after the server accepts it and **delivered** after it returns from an agent poll.
